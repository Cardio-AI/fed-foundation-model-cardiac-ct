...
def augment_gamma(data_sample, gamma_range=(0.5, 2), invert_image=False, epsilon=1e-7, per_channel=False,
    ...
    if not per_channel:
        ...
        data_sample = torch.pow(((data_sample - minm) / float(rnge + epsilon)), gamma) * rnge + minm
        if retain_stats_here:
        ...
        for c in range(data_sample.shape[0]):
            ...
            data_sample[c] = torch.pow(((data_sample[c] - minm) / float(rnge + epsilon)), gamma) * float(rnge + epsilon) + minm
            if retain_stats_here:
